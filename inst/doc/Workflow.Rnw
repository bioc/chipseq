...
%\VignetteDepends{Biobase, IRanges, GenomicFeatures}
...
<<>>=
plotCoverage(peaks.pos[wpeaks[1]], peaks.neg[wpeaks[1]])
@ 
...
<<>>=
plotCoverage(peaks.pos[wpeaks[2]], peaks.neg[wpeaks[2]])
@ 
<<fig=TRUE,height=5,echo=FALSE>>=
plot(trellis.last.object())
@ 
...
<<>>=
plotCoverage(peaks.pos[wpeaks[3]], peaks.neg[wpeaks[3]])
@ 
<<fig=TRUE,height=5,echo=FALSE>>=
plot(trellis.last.object())
@ 
...
<<>>=
plotCoverage(peaks.pos[wpeaks[4]], peaks.neg[wpeaks[4]])
@ 
<<fig=TRUE,height=5,echo=FALSE>>=
plot(trellis.last.object())
@ 
