...
cstest <- seqapply(cstest, function(gd) {
  gr <- do.call(c, lapply(names(gd), function(chr) {
    starts <- gd[[chr]]
    GRanges(chr, IRanges(unlist(starts), width = 24), 
            rep(names(starts), elementLengths(starts)))
  }))
})
@ 
%
...
function to a \class{Sequence} and returns the result as another
\class{Sequence}, if possible. In this case, our input
\class{Sequence} is \class{GRangesList}, and our expected output is a
\class{DataFrameList}.  Here is a simple summary function that
...
returned \class{DataFrameList} with the \code{stack} function.
<<>>=
