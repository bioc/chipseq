             ...
             chrlens = seqlengths(Mmusculus))
{
    ...
    corSummary <- function()
    {
        ...
        list(all = fcorrelation(simPeakSummary.df[, names(edata)]),
             promoter = fcorrelation(simPeakSummary.df[simPeakSummary.df$promoter, names(edata)]))
    }    
    cors <- replicate(10, corSummary(), simplify = FALSE)
    close(pb)
    list(peakSummary = peakSummary.df, 
         cors.obs = list(all = fcorrelation(peakSummary.df[, names(edata)]),
                         promoter = fcorrelation(peakSummary.df[peakSummary.df$promoter, names(edata)])),
         cors.null = cors, 
