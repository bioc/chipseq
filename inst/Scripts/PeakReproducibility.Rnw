...
commonPeaks <- function(lower = 10, x = pairedReads$"2", ref.peaks)
{
    ...
    count.overlap <- function(chr) 
    {
        ...
        {
            ov.fp <- 
                overlap(ref.peaks[[chr]], x.peaks[[chr]], multiple = FALSE)
            ov.fn <- 
                overlap(x.peaks[[chr]], ref.peaks[[chr]], multiple = FALSE)
        }
...
depthUnderPeaks <- function(x = pairedReads$"2", ref.peaks)
{
    get.depth <- function(chr) 
    {
        ref <- ref.peaks[[chr]]
        g <- extendReads(x[[chr]])
        cov <- coverage(g, 1, max(end(g), end(ref)))
        vcopy <- copyIRanges(ref, cov)
